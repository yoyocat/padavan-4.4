...
# 【下载广告过滤文件】
    if [ $(nvram get sdns_adblock) = "1" ]; then
        curl -k -s -o /tmp/sdnsadnew.conf --connect-timeout 10 --retry 3 $(nvram get sdns_adblock_url)
    fi
    if [ ! -f "/tmp/sdnsadnew.conf" ]; then
        logger -t "SmartDNS" "广告过滤功能未开启或者过滤地址失效,网络异常等！"
    else
        logger -t "SmartDNS" "AD文件下载成功,广告过滤功能已启用"
        if [ -f "/tmp/sdnsadnew.conf" ]; then
...
Start_smartdns () {
    ...
    if [ "$dnsmasq_md5" != $(md5sum  "$dnsmasq_Conf" | awk '{ print $1 }') ] ; then
         logger -t "SmartDNS" "重启 dnsmasq 服务器"
         /sbin/restart_dhcpd >/dev/null 2>&1
         logger -t "SmartDNS" "已重启成功。"
    fi
    # 启动 smartdns 进程
